...
def user_logged_handler(
    ...
    """Records successful login and zero fail counters."""
    ...
    security_logger.info(
        f"Login failure: {user.username} | IP: {ip} | User-Agent: {user_agent}",
        extra={"username": user.username, "ip": ip, "user_agen": user_agent},
    )


@receiver(user_login_failed)
def user_login_failad_handler(
    sender, credentials, request: HttpRequest, **kwargs
) -> None:
    """Controls login failures to prevent brute force."""
    ...
    try:
        ...
        if user.failed_login_attemps >= 5:
            ...
            security_logger.warning(
                f"Account temporarily blocked: {username} by {lock_minutes} minutes",
